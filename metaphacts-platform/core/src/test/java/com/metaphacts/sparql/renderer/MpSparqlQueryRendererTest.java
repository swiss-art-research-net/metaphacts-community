        ...
        if (parsedOp instanceof ParsedGraphQuery) {
            ...
            for (UpdateExpr update : exprs) {
                if (update instanceof Modify) {
                    optimizer.optimize(((Modify)update).getWhereExpr(), null, null);
                    ((Modify)update).setWhereExpr(optimizer.getOptimized());
                }
            }
        }
