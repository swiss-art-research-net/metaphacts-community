...
/**
 * Namespace PROV.
 * Prefix: {@code <http://www.w3.org/ns/prov#>}
 *
...
public class PROV {
	 ...
	 * another agent. The subordinate agent acted on behalf of the
	 * responsible agent in an actual activity. 
	 *
	 ...
	 * ActivityInfluence use the prov:activity property to cite the
	 * influencing Activity.
	 *
	 * @see <a href="http://www.w3.org/ns/prov#ActivityInfluence">ActivityInfluence</a>
	 */
	 ...
	 * the binary prov:wasAssociatedWith relation from an prov:Activity to
	 * some prov:Agent that had some responsiblity for it. For example,
	 * :baking prov:wasAssociatedWith :baker; prov:qualifiedAssociation [ a
	 ...
	 * recordings, etc.) that are not expressed in PROV-O, but can be still
	 * be described by PROV-O.
	 *
	 ...
	 * 'starting-point', 'qualifed', and 'extended'. This classification is
	 * used by the prov-o html document to gently introduce prov-o terms to
	 * its users. 
	 *
	 ...
	 * <p>
	 * An instance of prov:Communication provides additional descriptions
	 * about the binary prov:wasInformedBy relation from an informed
	 * prov:Activity to the prov:Activity that informed it. For example,
	 * :you_jumping_off_bridge prov:wasInformedBy
	 * :everyone_else_jumping_off_bridge; prov:qualifiedCommunication [ a
	 * prov:Communication; prov:activity :everyone_else_jumping_off_bridge;
	 * :foo :bar ].
	 *
	 ...
	 * classification is used so that readers of prov-o specification can
	 * find its correspondence with the prov-dm specification.
	 *
	 ...
	 * <p>
	 * A definition quoted from PROV-DM or PROV-CONSTRAINTS that describes
	 * the concept expressed with this OWL term.
	 *
	 ...
	 * the binary prov:actedOnBehalfOf relation from a performing prov:Agent
	 * to some prov:Agent for whom it was performed. For example, :mixing
	 * prov:wasAssociatedWith :toddler . :toddler prov:actedOnBehalfOf
	 * :mother; prov:qualifiedDelegation [ a prov:Delegation; prov:entity
	 * :mother; :foo :bar ].
	 *
	 ...
	 * the binary prov:wasDerivedFrom relation from some derived prov:Entity
	 * to another prov:Entity from which it was derived. For example,
	 ...
	 * PROV-DM concept, or how it should be used in context of semantic web
	 * or linked data.
	 *
	 ...
	 * <p>
	 * When the prov-o term does not have a definition drawn from prov-dm,
	 * and the prov-o editor provides one.
	 *
	 * @see <a href="http://www.w3.org/ns/prov#editorsDefinition">editorsDefinition</a>
	 */
	 ...
	 * :buzzer; prov:qualifiedEnd [ a prov:End; prov:entity :buzzer; :foo
	 ...
	 * influence upon any other kind of resource. Instances of
	 * EntityInfluence use the prov:entity property to cite the influencing
	 * Entity.
	 *
	 ...
	 * <p>
	 * The time at which an entity was completely created and is available
	 * for use.
	 *
	 ...
	 * the binary prov:wasGeneratedBy relation from a generated prov:Entity
	 * to the prov:Activity that generated it. For example, :cake
	 ...
	 * invalidated, or was the responsibility of some Entity. This property
	 * is _not_ used by ActivityInfluence (use prov:activity instead).
	 *
	 ...
	 *
	 * @see <a href="http://www.w3.org/ns/prov#hadPrimarySource">hadPrimarySource</a>
	 */
	 ...
	 * <p>
	 * An instance of prov:Influence provides additional descriptions about
	 * the binary prov:wasInfluencedBy relation from some influenced
	 * Activity, Entity, or Agent to the influencing Activity, Entity, or
	 * Agent. For example, :stomach_ache prov:wasInfluencedBy :spoon;
	 ...
	 * marks a change in the world, in its activities and in its entities.
	 * The term 'event' is commonly used in process algebra with a similar
	 * meaning. Events represent communications or interactions; they are
	 * assumed to be atomic and instantaneous.
	 *
	 * @see <a href="http://www.w3.org/ns/prov#InstantaneousEvent">InstantaneousEvent</a>
	 */
	 ...
	 *
	 * @see <a href="http://www.w3.org/ns/prov#invalidatedAtTime">invalidatedAtTime</a>
	 */
	 ...
	 * <p>
	 * An instance of prov:Invalidation provides additional descriptions
	 * about the binary prov:wasInvalidatedBy relation from an invalidated
	 * prov:Entity to the prov:Activity that invalidated it. For example,
	 * :uncracked_egg prov:wasInvalidatedBy :baking;
	 * prov:qualifiedInvalidation [ a prov:Invalidation; prov:activity
	 * :baking; :foo :bar ].
	 *
	 ...
	 * within a prov:category, among properties applying to a particular
	 * class, etc.) is unspecified.
	 *
	 ...
	 * intended goals. Since plans may evolve over time, it may become
	 * necessary to track their provenance, so plans themselves are entities.
	 * Representing the plan explicitly in the provenance can be useful for
	 * various tasks: for example, to validate the execution as represented
	 * in the provenance record, to manage expectation failures, or to
	 ...
	 * <p>
	 * An instance of prov:PrimarySource provides additional descriptions
	 * about the binary prov:hadPrimarySource relation from some secondary
	 ...
	 *
	 * @see <a href="http://www.w3.org/ns/prov#qualifiedAssociation">qualifiedAssociation</a>
	 */
	 ...
	 * it was influenced using prov:qualifiedAttribution [ a
	 * prov:Attribution; prov:agent :ag; :foo :bar ].
	 *
	 * @see <a href="http://www.w3.org/ns/prov#qualifiedAttribution">qualifiedAttribution</a>
	 */
	 ...
	 * <p>
	 * If this Activity prov:wasInformedBy Activity :a, then it can qualify
	 * how it was influenced using prov:qualifiedCommunication [ a
	 * prov:Communication; prov:activity :a; :foo :bar ].
	 *
	 * @see <a href="http://www.w3.org/ns/prov#qualifiedCommunication">qualifiedCommunication</a>
	 */
	 ...
	 * :ag; :foo :bar ].
	 *
	 * @see <a href="http://www.w3.org/ns/prov#qualifiedDelegation">qualifiedDelegation</a>
	 */
	 ...
	 * <p>
	 * If this Entity prov:wasDerivedFrom Entity :e, then it can qualify how
	 * it was derived using prov:qualifiedDerivation [ a prov:Derivation;
	 * prov:entity :e; :foo :bar ].
	 *
	 * @see <a href="http://www.w3.org/ns/prov#qualifiedDerivation">qualifiedDerivation</a>
	 */
	 ...
	 * <p>
	 * If this Activity prov:wasEndedBy Entity :e1, then it can qualify how
	 * it was ended using prov:qualifiedEnd [ a prov:End; prov:entity :e1;
	 * :foo :bar ].
	 *
	 ...
	 * with the subclass of prov:Influence and the qualifying property that
	 * are used to qualify it. Example annotation: prov:wasGeneratedBy
	 * prov:qualifiedForm prov:qualifiedGeneration, prov:Generation . Then
	 * this unqualified assertion: :entity1 prov:wasGeneratedBy :activity1 .
	 * can be qualified by adding: :entity1 prov:qualifiedGeneration
	 * :entity1Gen . :entity1Gen a prov:Generation, prov:Influence;
	 * prov:activity :activity1; :customValue 1337 . Note how the value of
	 * the unqualified influence (prov:wasGeneratedBy :activity1) is mirrored
	 * as the value of the prov:activity (or prov:entity, or prov:agent)
	 * property on the influence class.
	 *
	 ...
	 *
	 * @see <a href="http://www.w3.org/ns/prov#qualifiedGeneration">qualifiedGeneration</a>
	 */
	 ...
	 *
	 * @see <a href="http://www.w3.org/ns/prov#qualifiedInfluence">qualifiedInfluence</a>
	 */
	 ...
	 *
	 * @see <a href="http://www.w3.org/ns/prov#qualifiedInvalidation">qualifiedInvalidation</a>
	 */
	 ...
	 * <p>
	 * If this Entity prov:hadPrimarySource Entity :e, then it can qualify
	 * how using prov:qualifiedPrimarySource [ a prov:PrimarySource;
	 * prov:entity :e; :foo :bar ].
	 *
	 * @see <a href="http://www.w3.org/ns/prov#qualifiedPrimarySource">qualifiedPrimarySource</a>
	 */
	 ...
	 * :bar ].
	 *
	 * @see <a href="http://www.w3.org/ns/prov#qualifiedQuotation">qualifiedQuotation</a>
	 */
	 ...
	 * <p>
	 * If this Entity prov:wasRevisionOf Entity :e, then it can qualify how
	 * it was revised using prov:qualifiedRevision [ a prov:Revision;
	 * prov:entity :e; :foo :bar ].
	 *
	 * @see <a href="http://www.w3.org/ns/prov#qualifiedRevision">qualifiedRevision</a>
	 */
	 ...
	 * <p>
	 * If this Activity prov:used Entity :e, then it can qualify how it used
	 * it using prov:qualifiedUsage [ a prov:Usage; prov:entity :e; :foo :bar
	 * ].
	 *
	 ...
	 * <p>
	 * An instance of prov:Quotation provides additional descriptions about
	 * the binary prov:wasQuotedFrom relation from some taken prov:Entity
	 * from an earlier, larger prov:Entity. For example,
	 * :here_is_looking_at_you_kid prov:wasQuotedFrom :casablanca_script;
	 * prov:qualifiedQuotation [ a prov:Quotation; prov:entity
	 * :casablanca_script; :foo :bar ].
	 *
	 ...
	 * <p>
	 * An instance of prov:Revision provides additional descriptions about
	 * the binary prov:wasRevisionOf relation from some newer prov:Entity to
	 * an earlier prov:Entity. For example, :draft_2 prov:wasRevisionOf
	 * :draft_1; prov:qualifiedRevision [ a prov:Revision; prov:entity
	 * :draft_1; :foo :bar ].
	 *
	 ...
	 *
	 * @see <a href="http://www.w3.org/ns/prov#sharesDefinitionWith">sharesDefinitionWith</a>
	 */
	 ...
	 *
	 * @see <a href="http://www.w3.org/ns/prov#specializationOf">specializationOf</a>
	 */
	 ...
	 * binary prov:wasStartedBy relation from some started prov:Activity to
	 * an prov:Entity that started it. For example, :foot_race
	 * prov:wasStartedBy :bang; prov:qualifiedStart [ a prov:Start;
	 ...
	 * <p>
	 * Classes and properties used to qualify relationships are annotated
	 * with prov:unqualifiedForm to indicate the property used to assert an
	 ...
	 * binary prov:used relation from some prov:Activity to an prov:Entity
	 * that it used. For example, :keynote prov:used :podium;
	 * prov:qualifiedUsage [ a prov:Usage; prov:entity :podium; :foo :bar ].
	 *
	 ...
	 * <p>
	 * A prov:Entity that was used by this prov:Activity. For example,
	 ...
	 *
	 * @see <a href="http://www.w3.org/ns/prov#wasAssociatedWith">wasAssociatedWith</a>
	 */
	 ...
	 * prov:wasQuotedFrom, prov:wasRevisionOf, prov:hadPrimarySource) should
	 ...
	 * <p>
	 * End is when an activity is deemed to have ended. An end may refer to
	 * an entity, known as trigger, that terminated the activity.
	 *
	 ...
	 *
	 * @see <a href="http://www.w3.org/ns/prov#wasInvalidatedBy">wasInvalidatedBy</a>
	 */
