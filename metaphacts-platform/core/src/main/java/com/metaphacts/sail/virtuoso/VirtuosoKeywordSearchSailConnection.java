...
/**
 * Custom {@link SailConnection} implementation to process full-text search queries for the Virtuoso
 * repository.
 * 
 * Assumes the predicates &lt;bif:contains&gt; and &lt;bif:score&gt; to be used to express the
 * full-text search clause. (see {@link VirtuosoKeywordSearchHandler}).
 * 
...
public class VirtuosoKeywordSearchSailConnection extends AbstractSailConnection {
    ...
    protected CloseableIteration<? extends BindingSet, QueryEvaluationException> evaluateInternal(
        ...
        if (extractor.containsKeywordClauses()) {
            return this.evaluateAtTarget(extractor.getTupleExpr(), dataset, bindings,
                    includeInferred);
        } else {
    ...
    protected CloseableIteration<? extends BindingSet, QueryEvaluationException> evaluateAtTarget(
            TupleExpr tupleExpr, Dataset dataset, BindingSet bindings, boolean includeInferred)
            throws SailException {
        ...
        try {
            return handler.evaluateKeywordSearchQuery(connection, tupleExpr, dataset,
                    includeInferred);
        } catch (Exception e) {
            throw new SailException(e);
        }
    ...
    protected CloseableIteration<? extends Statement, SailException> getStatementsInternal(
        ...
        throw new UnsupportedOperationException(
                "Cannot retrieve statements by the pattern [" + subj + "," + pred + "," + obj
                        + "]: only complete full text search clauses are accepted");
    }
