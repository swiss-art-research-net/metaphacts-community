 ...
 * @deprecated replaced with WikidataLookupService, scheduled for removal in future release
 */
@Deprecated
public class WikidataSailConnection extends AbstractRESTWrappingSailConnection {
    ...
    public static final IRI USE_LANGUAGE = VF
            .createIRI("http://www.metaphacts.com/ontology/useLanguage");
    ...
    protected RESTParametersHolder extractInputsAndOutputs(List<StatementPattern> stmtPatterns)
        ...
        stmtPatterns.stream()
                ...
                .forEach(stmtPattern -> {
                    res.getInputParameters().put("language",
                            stmtPattern.getObjectVar().getValue().stringValue());
                    res.getInputParameters().put("uselang",
                            stmtPattern.getObjectVar().getValue().stringValue());
                });

        stmtPatterns.stream()
                .filter(stmtPattern -> stmtPattern.getPredicateVar().getValue().equals(RDFS.LABEL)
                        && stmtPattern.getSubjectVar().getName().equals(subjVarName)
                        && !stmtPattern.getObjectVar().hasValue())
                .forEach(stmtPattern -> res.getOutputVariables().put(RDFS.LABEL,
                        stmtPattern.getObjectVar().getName()));

        stmtPatterns.stream()
                .filter(stmtPattern -> stmtPattern.getPredicateVar().getValue()
                        .equals(SCHEMA_ORG_DESCRIPTION)
                        && stmtPattern.getSubjectVar().getName().equals(subjVarName)
                        && !stmtPattern.getObjectVar().hasValue())
        ...
        stmtPatterns.stream()
                .filter(stmtPattern -> stmtPattern.getPredicateVar().getValue().equals(ORDER_NR)
                        && stmtPattern.getSubjectVar().getName().equals(subjVarName)
                        && !stmtPattern.getObjectVar().hasValue())
                .forEach(stmtPattern -> res.getOutputVariables().put(ORDER_NR,
                        stmtPattern.getObjectVar().getName()));
        ...
        try {
            ...
            for (Object resObj : resList) {
                ...
                if (parametersHolder.getOutputVariables().containsKey(SCHEMA_ORG_DESCRIPTION)) {
                    String strVal = resMap.containsKey("description")
                            ? (String) resMap.get("description") : "";
                    Literal litDesc = VF.createLiteral(strVal);
                    bs.addBinding(parametersHolder.getOutputVariables().get(SCHEMA_ORG_DESCRIPTION),
                            litDesc);
                }
