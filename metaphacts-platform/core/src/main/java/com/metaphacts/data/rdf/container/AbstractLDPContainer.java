...
public abstract class AbstractLDPContainer extends AbstractLDPResource implements LDPContainer {
    ...
    protected void add(PointedGraph pointedGraph, RepositoryConnection repConnection) throws RepositoryException {
        ...
        if (isSavedToStorage()) {
            try {
                ...
                throw new RepositoryException("Cannot save the object "
                        + pointedGraph.getPointer().stringValue() + " to storage: " + e.getMessage(),
                        e);
            }
        }
    ...
    public void update(PointedGraph pointedGraph) throws RepositoryException {
        try (RepositoryConnection connection = getRepository().getConnection()) {
            ...
            if (!AbstractLDPResource.class.isAssignableFrom(toDelete.getClass())) {
                throw new IllegalStateException("LDP Resource implementation "
                    + toDelete.getClass() + " must extend "
    ...
    /**
     * Updates LDP resource's graph by replacing label of the resource with a new one.
     *
     ...
     * {@link LDPContainer} class instance. The default implementation ensures
     * that the container will get properly typed with at least
     * {@link LDP#Resource} and {@link LDP#Container}.
     *
     ...
     *
     * @see com.metaphacts.data.rdf.container.LDPContainer#initialize()
     */
    @Override
    ...
    public boolean containsLDPResource(Resource resource) {
        ...
        final String query = "SELECT ?containedResource WHERE {"
                + "?container <http://www.w3.org/ns/ldp#contains> ?containedResource"
                + "} LIMIT 1";
        TupleQuery tq = null;
        try (RepositoryConnection con = getConnection()) {
            ...
            try(TupleQueryResult result = tq.evaluate()){
                ...
                return QueryResults.asList(result).size() > 0;
            }
        } catch (Exception e) {
            throw Throwables.propagate(e);
        }
