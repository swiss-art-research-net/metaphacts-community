...
/**
 * A SecretLookup is used to lookup a secret value from a {@link SecretResolver}. 
 * 
 * <p>
 * The lookup string may contain both a key name as well as a fallback value which 
 * is used if the key cannot be resolved.<br/>
 * The format of the lookup string is {@code ${key:fallback}) where {@code key} specifies
 * the key to look up using a {@link SecretResolver} and {@code fallback} the fallback
...
public class SecretLookup {
    ...
    /**
     * Returns the lookup key part from a {@code ${key:fallback}) secret key. 
     * @param value value from which to extract the key part 
     * @return key part or empty if the provided value does not contain a variable expression
     */
    ...
    /**
     * Returns the fallback part from a {@code ${key:fallback}) secret key. 
     * @param value value from which to extract the key part 
     * @return fallback part or empty if the provided value does not contain a variable expression
     */
