...
public class MpFederationStrategy extends FederationStrategy {
    ...
    public CloseableIteration<BindingSet, QueryEvaluationException> evaluate(NaryJoin join,
        ...
        if ((seeds.size() == 2) && useCompetingJoin) {
            ...
            ParallelTimeEstimatingPullBoundRankJoin prj = 
                    new ParallelTimeEstimatingPullBoundRankJoin(
                            this, seeds, unsortedArgs, bindings, Lists.newArrayList(), -1);
    ...
    public CloseableIteration<BindingSet, QueryEvaluationException> evaluateJoin(
        ...
        if (( rightArg instanceof MpOwnedTupleExpr) && useBoundJoin) {
            ParallelBoundJoinCursor tmp = new ParallelBoundJoinCursor(
                    this, leftIter, (MpOwnedTupleExpr) rightArg);
            federation.execute((Runnable) tmp);
    ...
    protected CloseableIteration<BindingSet, QueryEvaluationException> evaluateUsingBoundRankJoin(
            MpOwnedTupleExpr expr, BindingSet bindings) throws QueryEvaluationException {
        CloseableIteration<BindingSet, QueryEvaluationException> result = expr.evaluate(dataset,
                bindings);
        return result;
    ...
    public CloseableIteration<BindingSet, QueryEvaluationException> evaluate(Service service,
            BindingSet bindings) throws QueryEvaluationException {
        logger.trace("Evaluating service: " + service.toString() + " with bindings: "
                + bindings.toString());
        i++;
        CloseableIteration<BindingSet, QueryEvaluationException> iter = super.evaluate(service,
                bindings);
        logger.trace("Evaluated service: " + service.toString() + " with bindings: "
                + bindings.toString());
