...
public final class StoragePath {
    ...
    public static Optional<StoragePath> tryParse(String path) {
        return assertPathIsValid(path, false)
            ? Optional.of(new StoragePath(path))
            : Optional.empty();
    }
    ...
    public static boolean assertPathIsValid(String storagePath, boolean throwOnError) {
        if (storagePath.startsWith(SEPARATOR)) {
            ...
            throw new IllegalArgumentException(
                "Storage path must not start with a separator: \"" + storagePath + "\"");
        }
        if (storagePath.endsWith(SEPARATOR)) {
            if (!throwOnError) { return false; }
            throw new IllegalArgumentException(
                "Storage path must not end with a separator: \"" + storagePath + "\"");
        }
    ...
    public boolean isPrefixOf(StoragePath subPath) {
        ...
        return subPath.path.startsWith(path) && (
            subPath.path.length() == path.length() ||
            subPath.path.startsWith(SEPARATOR, path.length())
        );
    }
    ...
    public String getLastComponent() {
        int lastSeparatorIndex = path.lastIndexOf(SEPARATOR);
        return lastSeparatorIndex < 0 ? path :
            path.substring(lastSeparatorIndex + SEPARATOR.length());
    }
    ...
    public boolean hasExtension(String extension) {
        ...
        return path.endsWith(extension)
            ...
            && !path.endsWith(SEPARATOR + extension);
    }
     ...
     * @throws IllegalArgumentException when object path is not a canonical representation of an IRI
     */
