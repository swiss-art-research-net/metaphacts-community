...
public class CacheManager {
    ...
    /**
     * Deregister specified cache.
     * @param cacheId id of cache to remove
     */
    public void deregister(String cacheId) {
        logger.debug("Deregistering cache '{}' in CacheManager.", cacheId);
        instances.remove(cacheId);
    }
    
    /**
     * Check whether a cache with specified id is registered.
     * 
     * @param cacheId id of cache to check for
     * @return <code>true</code> if the cache is registered, <code>false</code> otherwise
     */
    public boolean isRegistered(String cacheId) {
        return instances.containsKey(cacheId);
    }
