...
import {
    Rdf, RdfParser, SerializedDiagram, convertToSerializedDiagram, parseTurtleText
} from '../../src/ontodia/index';

export class N3Parser implements RdfParser {
    parse(text: string) {
        return Promise.resolve(parseTurtleText(text, Rdf.OntodiaDataFactory));
    }
}

export function renderExample(example: JSX.Element): void {
