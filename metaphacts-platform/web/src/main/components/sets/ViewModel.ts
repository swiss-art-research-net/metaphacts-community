...
import {
  ...
  searchForSetsAndItems, fetchSetItems, createFilterPatterns, EditType,
} from './SetsModel';
...
export class ViewModel {
  ...
  templateForKind(kind: Rdf.Node, expectedToBeSet: boolean) {
    ...
    if (viewMode === 'list') {
      return config && config.listTemplate || (
        expectedToBeSet ? Defaults.SetListTemplate : Defaults.ItemListTemplate
      );
  ...
  loadSets(params: { keepItems: boolean; }) {
    ...
    this.loadingSets.map(
      this.rootSetIri().flatMap(rootSet => Kefir.combine([
        this.defaultSetIri(),
      ...
      value: ([defaultSet, loadedSets]) => {
        ...
        const openedSet = (state.openedSet && sets.has(state.openedSet.value))
  ...
  private loadSetItems(setIri: Rdf.Iri, params: { forceReload?: boolean } = {}) {
    ...
    if (set.items && !params.forceReload) { return; }
  ...
  openAndLoadSet(setIri: Rdf.Iri) {
    const state = this.getState();
    if (state.openedSet && state.openedSet.equals(setIri)) { return; }
  ...
  onDropItemToSet(item: Rdf.Iri, targetSet: Rdf.Iri) {
    this.cancellation.map(
      getSetServiceForUser(this.getContext()).flatMap(service =>
  ...
  onFilterChanged(
    textInput: boolean,
    searchText: string,
    filterValues: Immutable.List<FilterValue> | undefined,
  ) {
    ...
    .observe({
      value: results => {
        ...
        results.forEach(({items}) =>
          items.forEach(({iri}) => itemsIris.add(iri.value))
        );
    ...
    if (editing && editing.type === EditType.Create) {
      ...
      if (!editing.fetchingName && newName && newName !== editing.oldName) {
        ...
        const sets = state.sets.update(modifiedSet.iri.value, set => (
          {...set, editing: undefined, newName: undefined}
        ));
        this.setState({sets});
      }
    }
  ...
  private createNewSet(placeholderSetIri: Rdf.Iri, name: string) {
    ...
    this.setState({
      sets: sets.update(placeholderSetIri.value, set => (
        {...set, editing: {type: EditType.ApplyingChanges}}
      )),
    });

    this.cancellation.map(
      getSetServiceForUser(this.getContext())
        .flatMap(service => service.createSet(name)),
    ).observe({
      value: () => {
  ...
  private renameSet(setIri: Rdf.Iri, oldName: string, newName: string) {
    const {sets} = this.getState();
    this.setState({
      sets: sets.update(setIri.value, set => (
        {...set, editing: {type: EditType.ApplyingChanges}}
      )),
    });
    ...
    this.cancellation.map(
      getSetServiceForUser(this.getContext())
        .flatMap(service => service.renameResource(setIri, newName)),
    ).observe({
      value: () => {
        this.loadSets({keepItems: true});
  ...
  removeSet(set: Rdf.Iri) {
    this.cancellation.map(
      getSetServiceForUser(this.getContext()).flatMap(
        service => service.deleteResource(set)),
    ).observe({
      value: () => {
      ...
      value: oldName => {
        ...
        this.setState({
          sets: sets.update(targetSet.value, set => {
            if (!(set && set.editing)) { return set; }
            const {editing} = set;
            if (!(editing.type === EditType.Rename && editing.fetchingName)) { return set; }
  ...
  applyItemsOrder() {
    ...
    if (!displayedSet) { return; }

    const items = state.itemsOrdering.apply(displayedSet.items);
    this.setState({
      itemsOrdering: undefined,
      sets: state.sets.update(
        displayedSet.iri.value,
        set => ({...set, items, isLoading: true})
      ),
    });

    const holders = Immutable.List(
      items.map(item => ({holder: item.itemHolder, item: item.iri})),
    );

    this.cancellation.map(
      getSetServiceForUser(this.getContext())
        .flatMap(service => service.reorderItems(displayedSet.iri, holders)),
    ).observe({
      value: () => {
...
function reuseOldSetItems(
  ...
  if (!oldSets) { return newSets; }
  return newSets.map((set, key) => {
    const oldSet = oldSets.get(key);
    if (!oldSet || !oldSet.items || set.items) { return set; }
