...
import {
  createElement, createFactory, ReactElement, CSSProperties, ChangeEvent, FormEvent,
} from 'react';
...
export class PlainTextInput extends AtomicValueInput<PlainTextInputProps, State> {
  ...
  render() {
    return D.div(
      ...
      D.div({className: 'plain-text-field__inputs'},
        this.renderElement(),
  ...
  private renderElement(): ReactElement<any> {
    ...
    if (this.isMultiline()) {
      ...
      return createElement(FormGroup, {validationState: this.getStyle()},
