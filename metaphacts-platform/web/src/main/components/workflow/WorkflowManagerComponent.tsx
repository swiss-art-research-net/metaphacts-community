...
export class WorkflowManagerComponent extends Component<Props, State> {
  ...
  private fetchWorkflowInstantiations(props: Props) {
    ...
    this.loadingCancellation.map(
      workflowStatesAndStepsQuerying
    ).observe({
      value: ({steps, workflowState}) =>
        this.setState({workflowState, steps, error: undefined, status: Status.Ready}),
      error: error =>
        this.setState({error, status: Status.Ready}),
    });
  }
  ...
  private renderStepSelect() {
    const {workflowState: {step}, steps, status} = this.state;
  ...
  private renderAssigneeSelect() {
    const {workflowState: {assignee}, assignees, status} = this.state;
  ...
  render() {
    ...
    const workflowStateHasChanged =
      !WorkflowState.isEqual(this.initialWorkflowState, workflowState);
    return (
