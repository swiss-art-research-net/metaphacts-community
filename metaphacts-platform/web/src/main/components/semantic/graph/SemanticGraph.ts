  ...
  private catchCORSError(elements: Cy.ElementDefinition[]) {
    ...
    const loadingImages = thumbnails.map(thumbnail =>
      new Promise<void>((resolve, reject) => {
        ...
        img.onerror = () => {
          reject("Some images are not accessible or can't be rendered on canvas because of CORS or mixed content error (loading HTTP resource from HTTPS context)!");
        };
  ...
  private navigate = (event: Cy.EventObject) => {
    ...
    if (isIri && isNotCompound) {
      ...
      if (this.props.openLinksInNewWindow) {
        ...
        navigateToResource(
          event.cyTarget.data('node') as Rdf.Iri, {}, repository
        ).onValue(identity);
      }
    }
  }
