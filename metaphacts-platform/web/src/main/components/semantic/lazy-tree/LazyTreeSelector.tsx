...
export class LazyTreeSelector extends Component<LazyTreeSelectorProps, State> {
  ...
  private onItemCheckedChange(
    ...
    const next = selectionMode.change(
      forest,
      item,
      previous,
      itemSelection,
...
function computeEntries(props: LazyTreeSelectorProps): {
  ...
  describeChildren(
    props,
    props.forest.root,
    props.selection ? props.selection.root : undefined,
    undefined,
    0,
    entries,
  );
  ...
  if (item.children) {
    ...
    for (const child of item.children as ReadonlyArray<TreeNode>) {
      ...
      if (parentSelection) {
        ...
        itemSelection = typeof childIndex === 'number'
          ? parentSelection.children[childIndex] : undefined;
      }
...
function describeItem(
  ...
  if (expanded) {
    const defaultSelectionForChildren =
      itemSelection && TreeSelection.isTerminal(itemSelection)
        ? itemSelection : defaultSelection;
