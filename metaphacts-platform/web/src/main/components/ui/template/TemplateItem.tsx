...
export interface TemplateItemProps {
  ...
  componentMapper?: (component: React.ReactNode) => React.ReactNode;
}
...
interface State {
  ...
  renderedTemplate?: React.ReactElement<any> | React.ReactElement[];
  error?: any;
}
...
export class TemplateItem extends Component<TemplateItemProps, State> {
  ...
  render() {
    ...
    let component: React.ReactNode | React.ReactNode[];
    if (typeof root === 'string') {
      component = <span>{root}</span>;
    } else if (Array.isArray(root)) {
      component = root;
    } else if (root) {
      component = React.cloneElement(root, {
        ...componentProps,
        ...root.props,
    ...
    // use a spread on fragment to prevent React warning:
    // Each child in a list should have a unique "key" prop
    const componentFragment = Array.isArray(component)
      ? React.createElement(React.Fragment, {}, ...component)
      : component;
    return children ? <>{componentFragment}{children}</> : componentFragment;
  }
  ...
  private renderTemplate(props: TemplateItemProps, compiledTemplate: CompiledTemplate) {
    const dataContext = mergeInContextOverride(this.appliedDataContext, props.template.options);
    try {
...
function tryUnwrapSingleChild(parsed: React.ReactElement<any> | React.ReactElement<any>[]) {
