...
function overrideContextData(
  base: HandlebarsDataStack,
  override: HandlebarsDataStack
): HandlebarsDataStack {
...
export const DataContextFunctions = {
  ...
  expose: function (this: any, key: string, options: any) {
    ...
    const dataContext = capturedContext instanceof CapturedContext
      ? capturedContext.getContext(key) : undefined;

    if (dataContext) {
      return options.fn(
        mergeDataContext(dataContext.context, this),
