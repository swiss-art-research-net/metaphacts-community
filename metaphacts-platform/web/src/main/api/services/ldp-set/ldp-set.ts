...
export class SetService extends LdpService {
  ...
  addToExistingSet(
    setIri: Rdf.Iri, itemIris: Rdf.Iri
  ): Kefir.Property<HolderWithItem> {
    const existingSet = new LdpService(setIri.value, this.context);
    return addSetItem(existingSet, itemIris);
  }

  createSetAndAddItems(
    name: string, listOfItemIris: Immutable.List<Rdf.Iri>
  ): Kefir.Property<any[]> {
    return this.createSet(name).flatMap((setLocation: Rdf.Iri) => {
      const newSet = new LdpService(setLocation.value, this.context);
      return Kefir.zip(
        listOfItemIris.map((iri, index) => addSetItem(newSet, iri, index)).toJS()
      );
    }).toProperty();
  }
  ...
  reorderItems(setIri: Rdf.Iri, holders: Immutable.List<HolderWithItem>): Kefir.Property<void> {
    ...
    return Kefir.zip(
      holders.map(({holder, item}, index) =>
        createItemHolderGraph(holder, item, index).map(graph =>
          set.update(holder, graph)
        )
      ).toArray()
...
function addSetItem(
  set: LdpService, item: Rdf.Iri, index?: number
): Kefir.Property<HolderWithItem> {
  return createItemHolderGraph(Rdf.iri(''), item, index).flatMap(
    graph => set.addResource(graph)
  ).map(
    holder => ({holder, item})
  ).toProperty();
}

function createItemHolderGraph(
  holderIri: Rdf.Iri, itemIri: Rdf.Iri, index?: number
  ...
  return getLabel(itemIri).map(label => {
    ...
    if (typeof index === 'number') {
      triples.push(Rdf.triple(
        holderIri,
        VocabPlatform.setItemIndex,
        Rdf.literal(index.toString(), xsd.integer)
      ));
    }
...
export function addToDefaultSet(
  resource: Rdf.Iri, sourceId: string
): Kefir.Property<Rdf.Iri> {
  return Kefir.combine([
    getSetServiceForUser(),
    Kefir.fromPromise(getUserDefaultSetIri()),
  ]).flatMap(
    ([service, defaultSetIri]) =>
      service.addToExistingSet(
        defaultSetIri, resource
      ).map(() => {
        trigger(
          {eventType: SetManagementEvents.ItemAdded, source: sourceId}
        );
        return resource;
      })
  ).toProperty();
    ...
    return this.get(setContainerIri)
      ...
      .flatMapErrors(() =>
        Kefir.combine([
          ...
          this.createSetContainerForUser(user, setContainerIri).flatMap(() => setService
            .createSet(
              'Uncategorized',
              maybe.Just(defaultSetIri.value)
            ).flatMapErrors(() => Kefir.constant({}))
          )
        )
      ).map(() => setService).toProperty();
  }
