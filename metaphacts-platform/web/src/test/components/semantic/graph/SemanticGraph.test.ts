...
const W3C_THUMBNAIL = 'https://upload.wikimedia.org/wikipedia/commons/thumb/e/ed/W3C%C2%AE_Icon.svg/210px-W3C%C2%AE_Icon.svg.png';
...
describe('graph-widget', () => {
  ...
  describe('rendering with default graph-widget configuration', () => {
    ...
    it('default stylesheet is applied if no styles are provided in props', () => {
      ...
      expect(
        cytoscapeComponent.props().graphStyle
      ).to.be.deep.equal(SemanticGraph.DEFAULT_STYLE);
    });

    it('query executed and result is properly translated to Cytotscape format', () => {
      const expectedGraphData: any[] =
        [
          {
            'group': 'edges',
            'data': {
      ...
      assert.sameDeepMembers(
        cytoscapeComponent.props().elements, expectedGraphData
      );
    });
  });
});
