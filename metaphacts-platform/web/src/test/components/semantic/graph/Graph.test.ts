...
describe('cytoscape', () => {
  ...
  before(
    function(done) {
      ...
      setTimeout(() => {
        ...
        cytoscapeComponent.state().cytoscape.map(
          (cy: Cy.Instance) => cytoscape = cy
        );
        done();
      }, 3000);
    }
  ...
  it('use handlebars templates to access node values in stylesheets', () => {
     ...
     */
    expect(
      (cytoscape.getElementById(person.alice.toString()).renderedStyle() as any)['label']
    ).to.be.equal('Alice');
  });
  ...
  it('exact typed literal matching', () => {
     ...
     */
    expect(
      cytoscape.getElementById(Rdf.literal('2', vocabularies.xsd.integer).toString()).renderedStyle()['background-color']
    ).to.be.equal('yellow');
  });
  ...
  it('exact string literal matching', () => {
     ...
     */
    expect(
      cytoscape.getElementById(Rdf.literal('Alice').toString()).renderedStyle()['background-color']
    ).to.be.equal('green');
  });
  ...
  it('match node by outgoing property value', () => {
     ...
     * }
     */
    [person.bob.toString(), person.mike.toString()].forEach(
      id =>
        expect(
          cytoscape.getElementById(id).renderedStyle()['background-color']
        ).to.be.equal('red')
    );
  });
  ...
  it('conjunctive property matching with typed literal', () => {
     ...
     */
    expect(
      (cytoscape.getElementById(
        person.bob.toString()
      ).renderedStyle() as any)['label']
    ).to.be.equal('Node linking to literal');
  });
  ...
  it('conjunctive property matching with string literal', () => {
    ...
    expect(
      cytoscape.getElementById(
        person.alice.toString()
      ).renderedStyle()['border-color']
    ).to.be.equal('#D95E52');
  });
});
