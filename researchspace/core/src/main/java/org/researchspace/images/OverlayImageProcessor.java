...
public class OverlayImageProcessor {
     ...
     * that's researchspace-specific without having it registered in
     * global Guice configuration
     *
     * @return
     */
    ...
    public PointedGraph convertToCRMDig(PointedGraph pointedGraph) throws RepositoryException {
        ...
        Resource modificationEvent = Models.subject(model.filter(null, RDF.TYPE, CRMdig.D3_FORMAL_DERIVATION_CLASS)).get();
    ...
    void substituteDummyIRIs(IRI overlayedImageUri, Model model, Resource modificationEvent) {
        // we need to materialize list, or while modifying model changing it with replaceSubjectAndObjects
        // we'll get ConcurrentModificatioException
        Set<Value> paramsURIs = new HashSet<Value>(model.filter(modificationEvent, CRMdig.L13_USED_PARAMETERS_PROPERTY, null).objects());
        int paramNo = 1;
        for (Value v : paramsURIs) {
            ModelUtils.replaceSubjectAndObjects(model, (Resource) v, vf.createIRI(overlayedImageUri.toString() + "/param" + paramNo));
            paramNo++;
        }
        ModelUtils.replaceSubjectAndObjects(model, modificationEvent, vf.createIRI(overlayedImageUri.toString() + "/event"));
    }
    ...
    List<BlendingParam> getBlendingParams(final Model model, Resource modificationEvent) {
        ...
        List<BlendingParam> params = Lists.newArrayList(Collections2.transform(paramsURIs, new Function<Value, BlendingParam>() {
            @Nullable
            @Override
            public BlendingParam apply(@Nullable Value input) {
                ...
                param.image = Models.objectIRI(model.filter(paramResource, RSO.OVERLAY_IMAGESOURCE_PROPERTY, null)).get();
                param.order = Models.objectLiteral(model.filter(paramResource, RSO.OVERLAY_ORDER_PROPERTY, null)).get().intValue();
                param.opacity = Models.objectLiteral(model.filter(paramResource, RSO.OVERLAY_OPACITY_PROPERTY, null)).get().doubleValue();
                return param;
            }
        }));
