...
export class SemanticTimeline extends Component<SemanticTimelineProps, SemanticTimelineState> {
  ...
  private initGroups = () => {
    ...
    this.state.dataSet.forEach(({group}) => {
      if (!group || groupsMap[group] || undefinedGroupsMap[group]) { return; }
  ...
  private getGroupsLabels(
    groupsMap: {[id: string]: TimelineGroup}
  ): Kefir.Property<{[key: string]: string}> {
    const iris = Object.keys(groupsMap).map(id => Rdf.iri(id));
    return LabelsService.getLabels(iris).map(labels =>
      labels.mapKeys(k => k.value).toObject()
    );
  }
