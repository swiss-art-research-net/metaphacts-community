...
import {
  ResourceSelectorConfig, SemanticQueryBuilderConfig,
  SEMANTIC_SEARCH_VARIABLES, RESOURCE_SEGGESTIONS_VARIABLES, getConfigPatternForCategory,
  Resource as PatternResource, Text as PatternText, TreeSelectorConfig,
...
import {
  SearchStore, SearchState, DomainSelection, RangeSelection, RelationSelection,
  RelationTermSelection, ConjunctStep,
  ConjunctIndex, TermSelectionSearch, TextTermSelection,
  EditKinds, ExtendedDomainSelection, ExtendedRelationSelection,
  TermType,
} from './SearchStore';
import {
...
function SearchClause(
  {clause, id, children}: {id: string, clause: Model.Conjunct; children?: React.ReactNode}
) {
...
class QueryBuilder extends Component<SemanticQueryBuilderConfig, State> {
    ...
    context.searchProfileStore.map(
      profileStore => {
        ...
        if (!searchStore || isExtendedSearch) {
          ...
          searchStore.currentSearchState.onValue(
            currentState => this.setState({searchState: currentState})
          );
          searchStore.currentSearch.onValue(
            currentSearch => {
  ...
  render() {
    return <div className={styles.searchAreaHolder}>
      {this.renderChild()}
  ...
  private renderSearchState(searchState: SearchState) {
    if (searchState.kind === 'domain-selection') {
      ...
      return <div className={styles.searchClauseArea}>
  ...
  private renderSearch = (search: Model.Search, searchState: SearchState, isNested = false) => {
    ...
    return <div>
      ...
      <div className={styles.searchArea}
           style={{display: isSearchCollapsed ? 'none' : null}}>
        {
          existingClauses
        }
      </div>
  ...
  private searchSummary = (search?: Model.Search) => {
    ...
    return <div className={styles.searchSummaryHolder}>
      { !_.isUndefined(search) ? <i className={
           ...
           onClick={this.onSearchToggle} /> : null }
  ...
  private renderFullClause =
    (domain: Model.Category, clause: Model.Conjunct, searchState: SearchState) =>
    Model.matchConjunct({
  ...
  private renderDomain = (category: Model.Category, isNested = false) => {
    const domainElement =
      renderResource(this.props.categoryViewTemplate, category, () => {/**/}, styles.selectedDomain);
    return isNested ? this.withoutEdit(EditKinds.Domain)(domainElement) : this.withEdit(EditKinds.Domain)(domainElement);
  }

  private renderRange = (category: Model.Category, conjunct?: Model.Conjunct) =>
    this.withEdit(EditKinds.Range, conjunct)(
      renderResource(this.props.categoryViewTemplate, category, () => {/**/}, styles.selectedRange)
    );

  private renderRelation = (relation: Model.Relation, conjunct?: Model.Conjunct) =>
    this.withEdit(EditKinds.Relation, conjunct)(
      renderResource(
        this.props.relationViewTemplate, relation, () => {/**/}, styles.selectedRelation
      )
    );

  private renderSimpleTerms = (conjunct: Model.Conjunct) =>
    _.filter<Model.Disjunct>(
      conjunct.disjuncts, term => term.kind !== Model.EntityDisjunctKinds.Search
    ).map(this.renderSimpleTerm(conjunct));

  private renderSimpleTerm = (conjunct: Model.Conjunct) => (disjunct: Model.Disjunct) =>
    this.withEdit(EditKinds.Disjunct, conjunct, disjunct)(
  ...
  private renderNestedTerm = (term: Model.Disjunct, searchState: SearchState) => {
    switch (term.kind) {
      case Model.EntityDisjunctKinds.Search: return this.renderNestedSearch(term, searchState);
    }
  }

  private renderNestedSearch = (term: Model.SearchDisjunct, searchState: SearchState) => {
  return <div className={styles.nestedSearchHolder}>
  ...
  private addDisjunctionButton = (conjunct: Model.Conjunct) =>
    <div className={styles.addDisjunctButton}
         onClick={ (e) => {
  ...
  private addConjunctionButton = (conjunct: Model.Conjunct) =>
    <div className={styles.addConjunctButton}
         onClick={ (e) => {
  ...
  private removeConjunctionButton = (conjunct: Model.Conjunct) =>
    <button className={classnames('btn', 'btn-link', styles.removeConjunctButton)}
  ...
  private withEdit =
    (editKind: EditKinds, conjunct?: Model.Conjunct, disjunct?: Model.Disjunct) =>
    (element: React.ReactElement<any>) =>
      <div className={styles.itemHolder}>
        {element}
        <span className={classnames('fa fa-times-circle fa-lg', styles.editButton)}
  ...
  private renderExtendedDomainSelection = (searchState: ExtendedDomainSelection) =>
    <div className={styles.searchClause}>
      {this.categorySelection(
        searchState.domains, styles.domainSelection,
        this.state.store.selectExtendedDomain, 'search domain category selection'
      )}
  ...
  private renderActiveSearchClause(
    searchState: SearchState, isNestedSearch: boolean, clause?: Model.Conjunct
  ) {
    switch (searchState.kind) {
      ...
      case 'term-selection': return this.termSelection(
        searchState, isNestedSearch, clause as Model.RelationConjunct
      );
  ...
  private domainSelection(searchState: DomainSelection) {
    return this.categorySelection(
      searchState.domains, styles.domainSelection,
      this.state.store.selectDomain, 'search domain category selection'
    );
  }
  ...
  private rangeSelection(searchState: RangeSelection, isNesteadSearch: boolean) {
    return D.div(
      ...
      this.relationSelectorPlaceholder(),
      this.categorySelection(
        searchState.ranges, styles.rangeSelection,
        this.state.store.selectRange, 'search range category selection'
      ),
      this.removeActiveConjunctionButton()
    );
  }
  ...
  private termSelection(
    state: RelationTermSelection, isNestedSearch: boolean, clause?: Model.RelationConjunct,
  ) {
    return [
      <div className={styles.searchClauseHolder}>
        ...
        <div className={styles.activeTerm}>
          {this.termSelector(state, isNestedSearch)}
        </div>
  ...
  private textTermSelection(
    state: TextTermSelection, clause?: Model.TextConjunct, isNestedSearch?: boolean
  ) {
    return [
      <div className={styles.searchClauseHolder}>
          <div className={styles.searchClause}>
          ...
          <div className={styles.activeTerm}>
              {this.textDisjunctSelector(state)}
          </div>
      </div>,
    ];
  }

  private textSearchRelationPlaceholder = () =>
    <div className={styles.relationPlaceholder}>... text search ...</div>;
  ...
  private termSelector(searchState: RelationTermSelection, isNestedSearch: boolean) {
    if (_.includes(searchState.termKind, 'nested-search')) {
      ...
      return <div className={styles.searchBasedTermSelectorHolder}>
        {this.dateTermSelector(searchState)}
      </div>;
    } else {
  ...
  private resourceTermSelector = (searchState: RelationTermSelection) => {
    return <AutoCompletionInput className={styles.resourceSelector}
  ...
  private showMapSelection = () => {
    ...
    getOverlaySystem().show(
      ...
      <OverlayDialog
        show={true}
        type='lightbox'
        title='Set selection on map'
        onHide={onHide}>
        <MapSelectionOverlay suggestionConfig={this.props.geoSelector}
          onCancel={onHide}
  ...
  private onSearchAreaSelected = (area: SelectedArea) => {
    switch (area.type) {
      case SelectType.Box: this.state.store.selectTerm('place')(area.box); break;
      case SelectType.Circle: this.state.store.selectTerm('place')(area.circle); break;
    }
    getOverlaySystem().hide('map-selection');
  }

  private hierarchyTermSelector = (searchState: RelationTermSelection) => {
    return <SemanticTreeInput className={styles.hierarchySelector}
  ...
  private dateTermSelector = (searchState: RelationTermSelection) => {
    ...
    return <DateFormatSelector
      ...
      onChange={value =>
        trigger({eventType: SearchEvents.SearchDateFormatSelected, source, data: value})}
    />;
  }
  ...
  private droppableConfig = (searchState: RelationTermSelection) => {
    ...
    return {
      query: this.setClauseBindings(searchState, `
        ASK {
          {
  ...
  private prepareHierarchySelectorInputConfig(
    ...
    const hierarchySelectorConfig =
      this.isLightweightTreeConfig(selectorPatterns)
        ? createDefaultTreeQueries(selectorPatterns) : selectorPatterns;
    return _.assign(
      _.cloneDeep(hierarchySelectorConfig),
      {
        ...
        onSelectionChanged: (selection: TreeSelection<TreeNode>) => {
          ...
          this.selectResourceTerm('hierarchy')(
            iri, label.value, label.value, {}
          );
          trigger({
  ...
  private prepareAutoCompletionInputConfig(
    searchState: RelationTermSelection
  ...
  private selectResourceTerm = (type: TermType) => (
    iri: Rdf.Iri, label: string, description: string, tuple?: any
  ) => {
    if (_.includes(iri.value, 'container/queryContainer')) {
      QueryService().getQuery(iri).onValue(
        query => this.state.store.selectTerm(type)({
          query: SparqlUtil.parseQuerySync(query.value), label: query.label
        })
      );
    } else {
  ...
  private setClauseBindingsParsed(
    ...
    if (resourcePatternConfig) {
      ...
      patterns = SparqlUtil.parsePatterns(
        SearchDefaults.DefaultResourceSelectorRelationPattern, parsedQuery.prefixes);
    }
    new PatternBinder(SEMANTIC_SEARCH_VARIABLES.RELATION_PATTERN_VAR, patterns)
  ...
  private relationSelectorPlaceholder = () =>
    <div className={styles.relationPlaceholder}>... related to</div>;
