...
import {
  getCategoryTypes,
} from 'platform/components/semantic/search/data/search/ModelUtils';
...
export type SearchState =
  DomainSelection | RangeSelection | TextTermSelection |
  RelationSelection | TermSelection | View |
  ExtendedDomainSelection | ExtendedRelationSelection;

export type ActiveState =
  RangeSelection | TextTermSelection | RelationSelection | TermSelection;
...
export enum EditKinds {
  Domain, Range, Relation, Disjunct,
}
...
export class SearchStore {
   ...
   */
  private static SupportedCategoryTypes =
    ['resource', 'hierarchy', 'date-range', 'text', 'place', 'set'];
  ...
  private rangesFor = (domain: Model.Category): Model.Categories => {
    return this.filterOnlySupportedCategories(
      this._profileStore.rangesFor(domain)
    );
  }
  ...
  private filterOnlySupportedCategories = (categories: Model.Categories): Model.Categories => {
    return categories.filterNot(
      domain => {
        const supportedCategoryTypes =
          _.intersection(
            SearchStore.SupportedCategoryTypes, getCategoryTypes(this.config, domain)
          );
  ...
  public get currentSearchQuery(): Kefir.Property<Data.Maybe<SparqlJs.SelectQuery>> {
    return this._searchProperty.$property.map(
      maybeSearch => maybeSearch.map(
        search => generateSelectQuery(this.config, this.projectionVariable, search)
      )
    );
  }
    ...
    if (editingPersistantState) {
      ...
      case EditKinds.Range:
        if (conjunct.conjunctIndex.length > 1) {
          const baseConjunct =
            this.getConjunctByIndex(this._search.get(), _.dropRight(conjunct.conjunctIndex, 2)) as Model.RelationConjunct;
  ...
  private updateNestedState(
    currentState: ActiveState, newState: ActiveState
  ): ActiveState {
  ...
  private selectTextDisjunctState(
    state: RangeSelection, range: Model.Category
  ): TextTermSelection  {
  ...
  private selectRelationState(
    state: RangeSelection, range: Model.Category
  ): RelationSelection  {
    const { conjunctIndex } = this._searchState as ConjunctStep;
    const relations =
      this._profileStore.relationsFor({
  ...
  private _selectRelation = (state: ActiveState, relation: Model.Relation) => {
    ...
    return this.updateNestedState(
      state, this.selectTermState(deepestActiveState, relation)
    );
  }

  public selectTermState = (
    state: RelationSelection, relation: Model.Relation
  ): TermSelection => {
  ...
  private getDisjunctType = (
    searchState: TermSelection, termType: TermType, value: any
  ): Model.DisjunctKind => {
    if (searchState.kind === 'text-term-selection') {
  ...
  public selectTerm = (termType: TermType) => (value: any) => {
    ...
    if (this._search.isJust) {
      ...
      this.search = Maybe.Just(
        this.createInitialSearch(this._searchState as any, value, termType)
      );
    }
    this.searchState = null;
  }
    ...
    return {
      ...
      conjuncts: [{
        ...
        disjuncts: [{
          ...
          value: isNestedSearch ? this.createInitialSearch((state as any).state, resource, termType, n + 2) : this.getDisjunctValue(resource),
  ...
  private addRelationDisjunction =
