...
export class GuidedTour extends React.Component<Props, State> {
  ...
  private registerEventListener() {
    ...
    this.cancellation.map(
      listen({eventType: GuidedTourRunIfNotSeen, target})
    ).onValue(() => {
    ...
    this.cancellation.map(
      listen({eventType: GuidedTourStart, target})
    ).onValue(() => {
  ...
  private listenToStepsEvents() {
    const uniqueEvents = _.uniqWith(this.props.steps, (step: Step, otherStep: Step) =>
      step.progressEvent && otherStep.progressEvent &&
    ...
    uniqueEvents.forEach(({progressEvent}) => {
      ...
      this.cancellation.map(
        listen({eventType: progressEvent.eventType, source: progressEvent.source})
      ).onValue(event => {
