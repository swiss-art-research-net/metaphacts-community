...
export class ShareComponent extends Component<Props, State> {
  ...
  componentDidMount() {
    ...
    query
      ...
      .onError(
      error => this.setState({ visibility: visibility.private, errorMessage: maybe.Just(error) })
      );
  }
          ...
          v =>
            <div className='radio'>
              <label className='control-label' key={'visibility' + v}>
                <input type='radio' name='visibility' value={visibility[v]}
                  onClick={this.onClick} checked={this.isSelected(visibility[v])} />
                {_.upperFirst(v)}
              </label>
              {visibility[v] === visibility.group && this.isSelected(visibility.group) ? this.renderGroupSelector() : null}
            </div>
        )}
      </fieldset>
    </div>;
  ...
  renderGroupSelector = () =>
    <AutoCompletionInput
      ...
      query={DEFAULT_GROUPS_QUERY} defaultQuery={DEFAULT_GROUPS_QUERY}
