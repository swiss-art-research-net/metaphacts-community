...
import {
  OntodiaAnnotationEndpoint, OntodiaAnnotationEndpointFields, MiradorRegions,
...
export class ImageGraphAuthoringComponent extends Component<ImageGraphAuthoringConfig, State> {
    ...
    this.cancellation.map(
      ...
      value: ({data: {model, authoringState}}) => {
        ...
        const newMiradorRegions = this.findRegionsOnDiagram(
          model as DiagramModel, authoringState as AuthoringState
        );
  ...
  private shouldUpdate(
    miradorRegions: MiradorRegions,
    newMiradorRegions: MiradorRegions
  ): boolean {
    if (Object.keys(miradorRegions).length !== Object.keys(newMiradorRegions).length) {
      return true;
    }
    for (const imageIri in miradorRegions) {
      if (!miradorRegions.hasOwnProperty(imageIri)) { continue; }
  ...
  render() {
    ...
    if (isLoading || !Object.keys(miradorRegions).length) { return null; }

    const images = {
      ['http://www.researchspace.org/ontology/Image_Graph_Authoring']: Object.keys(miradorRegions),
    };
    return <ImageRegionEditorComponentMirador key={key}
      {...this.props}
