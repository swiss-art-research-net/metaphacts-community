...
export class OpenSeadragonOverlay extends Component<OverlayProps, {}> {
  ...
  private setOpacity(firstImageOpacity: number) {
    const subsequentOpacity = subsequentImagesOpacity(
      firstImageOpacity, this.osd.images.length);
  ...
  private onFirstImageLoaded = (event: OpenSeadragon.TileLoadedEvent) => {
    ...
    const opacity = subsequentImagesOpacity(
      this.props.firstImageWeight, this.props.metadata.size);
