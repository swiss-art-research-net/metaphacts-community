...
class LightBoxTable extends Component<LightboxTableProps, any> {
  ...
  render() {
        ...
        createElement(
            ...
            MenuItem, {
              ...
              onClick: () =>
                this.showDialogAction(
                  'Side-by-Side image comparison', SideBySide, this.props.config.iiifServerUrl
                ),
          ...
          createElement(
            MenuItem, {
              ...
              onClick: () =>
                this.showDialogAction(
                  'Image overlay', OverlayComparison, this.props.config.iiifServerUrl
                ),
            }, 'Overlay'),
    ...
    return D.div(
      {},
      actions,
      createElement(SemanticTable, assign({ ref: 'table' }, this.props) as any)
    );
  }
  ...
  private showDialogAction = (title: string, component: any, iiifServerUrl: string) => {
    const images = this.getSelectedItems().map(node => node.value).toArray();
  ...
  private showCreateNewSetDialog = () => {
    ...
    getOverlaySystem().show(
      ...
      createElement(SaveSetDialog, {
        onSave: (name: string) => getSetServiceForUser().flatMap(service =>
          service.createSetAndAddItems(name, images)
        ),
        onHide: () => getOverlaySystem().hide(dialogRef),
