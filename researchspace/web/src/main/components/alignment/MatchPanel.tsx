...
import {
  AlignmentNode, AlignKind, findExcludedChildren,
...
export class MatchPanel extends Component<MatchPanelProps, {}> {
  ...
  private renderTreeNodeRow(
    node: AlignmentNode,
    highlightedNodes: ReadonlyArray<AlignmentNode>
  ) {
  ...
  private renderTreeNode(node: AlignmentNode, decoratorsClass: string) {
    const {role, onDragEnd, state: {forest, selection}} = this.props;
    ...
    if (role === AlignementRole.Source) {
      ...
      if (node.base) {
        const path = forest.getKeyPath(node);
        return (
          <Droppable onDrop={iri => this.onDropInto(path, iri)}>
            {body}
          </Droppable>
        );
      }
  ...
  private computeDecoratorsClass(
    item: AlignmentNode,
    highlightedNodes: ReadonlyArray<AlignmentNode>
  ): string {
    ...
    if (pathIndex >= 0) {
      ...
      classes.push(isTarget
        ? styles.decorateHighlightLeaf
        : styles.decorateHighlightParent
      );
    }
  ...
  private renderExpandToScrollMessage() {
    const {onCancelExpandingToScroll, state: {expandTarget}} = this.props;
    return (
  ...
  private requestMore = (node: AlignmentNode): void => {
    const {requestMoreItems, state: {forest}} = this.props;
    const path = forest.getKeyPath(node);
    requestMoreItems(path);
  }

  private onExpandedOrCollapsed = (item: AlignmentNode, expanded: boolean) => {
    const {onExpandOrCollapse, state: {forest}} = this.props;
  ...
  private renderUnalignButton(node: AlignmentNode) {
    const {onUnalign, state: {forest}} = this.props;
    if (!onUnalign) { return null; }
...
function renderAlignedTerm(node: AlignmentNode) {
  if (node.base && node.aligned) {
    ...
    return (
      <span className={styles.baseTerm}>
        {Node.getLabel(node.base)}
      </span>
    );
...
class NodeInfoPopover extends React.Component<NodeInfoPopoverProps, {}> {
  render() {
    ...
    const infoHeader = (
      <ResourceLink resource={targetNode.iri}>
        {nodeLabel}
      </ResourceLink>
    );
    const popoverProps = delegatedProps as any;
    return (
      <Popover id='alignment-tool-node-info'
        {...popoverProps}
