...
export class MatchList extends Component<MatchListProps, {}> {
  render() {
    ...
    return (
      <ul className={classnames(styles.component, className)}>
        {entries.map(entry => this.renderMatch(entry))}
      </ul>
    );
  }
  ...
  private renderMatch(entry: MatchEntry): ReactElement<any> {
    ...
    const hasBeenSaved =
      this.props.savedMatches.hasIn(
        [entry.targetBase.base.iri.value, entry.targetAligned.aligned.iri.value]
      );
    return (
...
function computeMatchEntries(
  grouped: Immutable.Map<string, Immutable.Map<string, MatchEntry>>
): MatchEntry[] {
