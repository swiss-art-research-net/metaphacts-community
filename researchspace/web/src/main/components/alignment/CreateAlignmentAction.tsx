...
} from 'platform/components/ui/selection/SelectionActionComponent';
...
class CreateAlignmentDialog extends React.Component<DialogProps, AlignmentMetadata> {
  ...
  onCreate = () => {
    ...
    service.addResource(alignment, this.state.iri.map(i => i.value)).flatMap(alignmentIri => {
      return navigateToResource(alignmentIri, {}, 'assets');
    }).onValue(() => {/**/});
  }

  private swapTerminologies = () =>
    this.setState(({source, target}) => ({source: target, target: source}));
}
...
export class CreateAlignmentAction extends Component<CreateAlignmentActionProps, ActionState> {
  ...
  onAction = (selection: Array<string>) => {
    getOverlaySystem().show(
      ACTION_DIALOG_REF,
      <CreateAlignmentDialog state={this.state} />
    );
  }
}
