...
import {
  KeyedForest, KeyPath, TreeNode, Node, SparqlNodeModel, TreeSelection, Traversable,
  SelectionNode, nodesFromQueryResult, mergeRemovingDuplicates,
...
export namespace AlignmentNode {
  ...
  export const keyOf = (node: AlignmentNode) => {
    return (
      node.base ? Node.keyOf(node.base) :
      node.aligned ? Node.keyOf(node.aligned) :
      rootKey
    );
  };
  ...
  export function addNarrowMatches(
    target: AlignmentNode, matches: ReadonlyArray<AlignmentNode>
  ): AlignmentNode {
...
export class AlignmentNodeModel {
  ...
  hasMoreChildren(node: AlignmentNode): boolean {
    return node.base && this.baseModel.hasMoreChildren(node.base) ||
  ...
  loadMoreChildren(parent: AlignmentNode): Kefir.Property<AlignmentNode> {
    ...
    return Kefir.combine({
      base: (base && !base.reachedLimit)
        ? this.baseModel.loadMoreChildren(base) : Kefir.constant(undefined),
      aligned: (aligned && !aligned.reachedLimit)
        ? this.alignedModel.loadMoreChildren(aligned) : Kefir.constant(undefined),
    }).map(loaded => {
      let result = parent;
      ...
      const error = result.base && result.base.error ||
        result.aligned && result.aligned.error ||
        undefined;
    ...
    function matchTargetNodes(targetTree: AlignmentNode, sourceLeafs: Immutable.Map<string, Node>) {
      ...
      state.matches.forEach((matches, targetKey) => {
        ...
        result.nodes.get(targetKey).forEach(target => {
          result = result.updateNode(
            result.getKeyPath(target),
            node => {
              let aligned = node;
              if (exact) {
                aligned = AlignmentNode.setExactMatch(
                  aligned, sourceLeafs.get(exact.iri.value), exact.excluded
                );
              }
              aligned = AlignmentNode.addNarrowMatches(aligned, narrow.map(match =>
                asNarrowMatch(sourceLeafs.get(match.iri.value), match.excluded)
              ));
              return aligned;
            }
          );
  ...
  private loadSkeleton(params: { iris: ReadonlyArray<Rdf.Iri>, useBaseModel: boolean }) {
    ...
    return loadNodesUsingNodeQuery(params.iris, nodeQuery, model.sparqlOptions())
      .flatMap(leafs => {
        return model.loadFromLeafs(leafs.toArray(), {transitiveReduction: false})
          .map(asBaseNode)
          .map(tree => ({leafs, tree}));
      });
  }
...
export function unalignAll(
  ...
  while (true) {
    ...
    result = unaligned
      ? result.updateNode(path, () => unaligned)
      : result.removeNode(path);
  }
  return result;
}

function findFirstAlignmentTarget(
  forest: KeyedForest<AlignmentNode>,
  sourceKey: string,
  targetKey: string
) {
  const emptyNodes = Immutable.Set<AlignmentNode>();
  const narrowMatch = forest.nodes.get(sourceKey, emptyNodes)
    .find(node => node.alignKind === AlignKind.NarrowerMatch);
  if (narrowMatch) { return narrowMatch; }
  const exactMatch = forest.nodes.get(targetKey, emptyNodes)
    .find(node => node.aligned
      && Node.keyOf(node.aligned) === sourceKey
      && node.alignKind === AlignKind.ExactMatch);
  return exactMatch;
}
...
function asMatchChildren(
  ...
  return (children || [])
    ...
    .map(node => AlignmentNode.set(AlignmentNode.empty, {
      aligned: node,
...
function mergeChildren(
  oldNodes: ReadonlyArray<AlignmentNode>,
  newNodes: ReadonlyArray<AlignmentNode>
) {
