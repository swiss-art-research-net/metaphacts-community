...
import { rso } from 'researchspace/data/vocabularies';

import { KeyedForest } from 'platform/components/semantic/lazy-tree';

import {
  AlignmentNode, AlignmentState, AlignmentMatch, AlignKind, AlignmentMetadata,
...
export function groupMatches(
  ...
  return Immutable.Map<string, Immutable.Map<string, MatchEntry>>()
    .withMutations(matchesByTarget => {
      forest.nodes.forEach((nodes, key) => {
        nodes.forEach(targetAligned => {
          const kind = targetAligned.alignKind;
          if (targetAligned.aligned && (
            kind === AlignKind.ExactMatch ||
            ...
            matchesByTarget.update(targetKey, matchesBySource => {
              ...
              if (existing.has(sourceKey)) { return existing; }
...
export function exportAlignment(
  forest: KeyedForest<AlignmentNode>, metadata: AlignmentMetadata
): AlignmentState {
  const matches = groupMatches(forest).map(values =>
    values.map(({kind, targetAligned}): AlignmentMatch => ({
      kind,
