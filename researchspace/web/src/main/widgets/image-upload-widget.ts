...
import {
  FieldDefinitionProp, FieldValue, CompositeValue,
  PlainTextInput, PlainTextInputProps, SelectInput, AutocompleteInput,
  DatePickerInput, DatePickerInputProps, ResourceEditorForm, ResourceEditorFormProps,
...
class ImageUploadWidget extends Component<Props, State> {
  ...
  private submit(fieldValues: CompositeValue): Kefir.Property<void> {
    ...
    const createResourceQuery = SparqlUtil.serializeQuery(
      parametrizeQueryWithFormValues(
        this.props.config.createResourceQuery, fieldValues
      )
    );
    ...
    return FileUploadService.uploadFile({
      ...
      onProgress: percent => this.setState(assign({}, this.state, {
        progress: percent,
      })),
    }).map(newImageIri => {
    ...
    return D.div({className: 'iiif-upload__holder'},
      ...
      createElement(ResourceEditorForm,
        ...
        React.createElement<PlainTextInputProps>(
          PlainTextInput, {for: 'title', placeholder: 'Type title here'}),
        React.createElement(AutocompleteInput, {for: 'person'}),
        React.createElement<DatePickerInputProps>(
